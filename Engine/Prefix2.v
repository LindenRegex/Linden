...
Fixpoint extract_literals_char (cd: char_descr) : list literal :=
  ...
  (* | CdRange l h => map (fun x => Exact [x]) (CharSet.elements (CharSet.range l h)) *)
  | CdUnion cd1 cd2 => extract_literals_char cd1 ++ extract_literals_char cd2
  | CdDigits => [Unknown]
  (* | CdDigits => map (fun x => Exact [x]) (CharSet.elements Characters.digits) *)
  | CdWhitespace => [Unknown]
  (* | CdWhitespace => map (fun x => Exact [x]) (CharSet.elements Characters.white_spaces) *)
  | CdDot | CdAll | CdNonDigits | CdNonWhitespace | CdWordChar
...
*)
Fixpoint extract_literals (r: regex) : list literal :=
  (* FIXME: how to get all casings of a character? *)
  ...
  | Group _ r1 => extract_literals r1
  | Anchor _ => [Nothing]
...
Theorem extract_literal_char_impossible:
  forall r,
    extract_literal_char r = Impossible ->
    is_impossible (extract_literals_char r).
Proof.
...
Theorem extract_literal_impossible:
  forall r,
    ...
    is_impossible (extract_literals r).
Proof.
  unfold is_impossible.
...
Lemma extract_literal_char_impossible_no_match:
  forall cd c,
    is_impossible (extract_literals_char cd) ->
...
Lemma extract_literal_impossible_general:
  forall acts tree inp gm gm',
    ...
    is_impossible (extract_actions_literals acts) ->
    tree_res tree gm inp forward = None.
Proof.
  unfold is_impossible.
  intros acts tree inp gm gm' Htree.
  remember (forward) as dir.
